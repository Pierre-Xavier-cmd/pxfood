...
describe("Test Users API", () => {
    ...
    it("should register a normal user", (done) => {
        ...
        .expect(201)
        .end((err, res) => {
            createdUserId = res.body._doc._id
    ...
    it("should return a user by id", (done) => {
        ...
        .end((err, res) => {    
            ...
            assert(user, "Expected user object")
            assert(user._id, "Expected user to have an '_id' property")
            assert(user.email, "Expected user to have an 'email' property")
            assert(user.username, "Expected user to have an 'username' property")
            assert(user.password, "Expected user to have a 'password' property")
            assert(user.role, "Expected user to have a 'role' property")
    ...
    it("should update a user by id", (done) => {
        ...
        const request = supertest(app)
        request.put(`/api/users/${createdUserId}`).set("authorization", `Bearer ${userToken}`)
        .send(userUpdate)
        .expect(200)
        .end((err, res) => {
            if (err) {
            ...
            assert.strictEqual(res.body.username, "updated_username", "Expected user to have the updated username")
            done()
        })
    })
    ...
    it("should delete a user by id", (done) => {
        const request = supertest(app)
        request.delete(`/api/users/${createdUserId}`).set("authorization", `Bearer ${userToken}`)
        .expect(200)
        .end((err, res) => {
            if (err) {
                console.log(err)
